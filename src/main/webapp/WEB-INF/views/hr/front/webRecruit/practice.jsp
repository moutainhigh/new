    ...
    <div class="container">
        ...
        <div class="r_nav">
            <ul>
                ...
                <li><a class="active" href="<c:url value="/recruitment/practice"/>"><i class="internship"></i>实习生招聘</a></li>
...
<div class="r_sider_wrapper wrapper-width">
    <div class="r_slide_box1">
        <div class="ggBox"><img src="<c:url value="/resources/hr/front/images/bannar3.jpg"/>"></div>
    </div>
</div>
<!--招聘-->
<div class="center_box wrapper-width">
    <div class="container">
        <div class="r_url"><i class="icon-home"></i>首页 > 人才发展 > 实习生招聘</div>
        <div class="content_box">
            <div class="left_nav accordion">
                <ul>
                    <li <c:if test="${p eq 0}">class="current"</c:if>>
                        <a href="<c:url value="/recruitment/practice/${0}/${1}/0"/>"><span class="menu_link">集团总部</span></a>
                    ...
                    <li <c:if test="${p eq 1}">class="current"</c:if>>
                        ...
                        <div class="sub_menu" <c:if test="${p eq 1}">style="display:block;"</c:if>>
                            <a <c:if test="${p eq 1 && a eq 1}">class="curr"<c:set value="重庆区域" var="areaStr"/></c:if> href="<c:url value="/recruitment/practice/${1}/${1}/0"/>">重庆区域公司</a>
                            <a <c:if test="${p eq 1 && a eq 2}">class="curr"<c:set value="四川区域" var="areaStr"/></c:if> href="<c:url value="/recruitment/practice/${1}/${2}/0"/>">四川区域公司</a>
                            <a <c:if test="${p eq 1 && a eq 3}">class="curr"<c:set value="华东区域" var="areaStr"/></c:if> href="<c:url value="/recruitment/practice/${1}/${3}/0"/>">华东区域公司</a>
                            <a <c:if test="${p eq 1 && a eq 5}">class="curr"<c:set value="长沙城市公司" var="areaStr"/></c:if> href="<c:url value="/recruitment/practice/${1}/${5}/0"/>">长沙城市公司</a>
                            <a <c:if test="${p eq 1 && a eq 6}">class="curr"<c:set value="沈阳城市公司" var="areaStr"/></c:if> href="<c:url value="/recruitment/practice/${1}/${6}/0"/>">沈阳城市公司</a>
                        </div>
                    </li>
                    <li <c:if test="${p eq 2}">class="current"</c:if>>
                        ...
                        <div class="sub_menu" <c:if test="${p eq 2}">style="display:block;"</c:if>>
                            <a <c:if test="${p eq 2 && a eq 1}">class="curr"<c:set value="重庆区域" var="areaStr"/></c:if> href="<c:url value="/recruitment/practice/${2}/${1}/0"/>">重庆区域公司</a>
                            <a <c:if test="${p eq 2 && a eq 2}">class="curr"<c:set value="四川区域" var="areaStr"/></c:if> href="<c:url value="/recruitment/practice/${2}/${2}/0"/>">四川区域公司</a>
                            <a <c:if test="${p eq 2 && a eq 3}">class="curr"<c:set value="华东区域" var="areaStr"/></c:if> href="<c:url value="/recruitment/practice/${2}/${3}/0"/>">华东区域公司</a>
                        </div>
                    </li>
                    <li <c:if test="${p eq 3}">class="current"</c:if>>
                        <a href="<c:url value="/recruitment/practice/${3}/${1}/0"/>"><span class="menu_link">金控集团</span></a>
                        <div class="sub_menu"></div>
                    </li>
                    <li <c:if test="${p eq 4}">class="current"</c:if>>
                        <a href="<c:url value="/recruitment/practice/${4}/${1}/0"/>"><span class="menu_link">建设集团</span></a>
                        <div class="sub_menu"></div>
                    </li>
                    <li <c:if test="${p eq 5}">class="current"</c:if>>
                        <a href="<c:url value="/recruitment/practice/${5}/${1}/0"/>"><span class="menu_link">物业集团</span></a>
                        <div class="sub_menu"></div>
                    </li>
                    <li <c:if test="${p eq 6}">class="current"</c:if>>
                        <a href="<c:url value="/recruitment/practice/${6}/${1}/0"/>"><span class="menu_link">海外公司</span></a>
                        <div class="sub_menu"></div>
                    </li>
                </ul>
            </div>
                ...
                <ul>
                    <c:forEach items="${page.content}" var="entity">
                        <li>
                            ...
                            <div class="re_text">
                                    ${entity.allText}
                                <div class="resume"><b>请发送简历至hyrencai@126.com或者<a class="orange" href="<c:url value="/recruitment/applyJob?cid=${entity.plateId}"/>">在线投递简历</a></b></div>
                            </div>
                        </li>
                    </c:forEach>
                </ul>
            ...
            <div class="re_page">
                <ul>
                    <li><a href="<c:url value="/recruitment/practice"/>"><i class="icon-step-backward"></i></a></li>
                    <c:if test="${page.hasPrevious()}">
                        <li><a href="<c:url value="/recruitment/practice/${p}/${a}/${page.number-1}"/>"><i class="icon-backward"></i></a></li>
                    </c:if>
                    <c:if test="${!page.hasPrevious()}">
                    ...
                    <c:if test="${page.hasNext()}">
                        <li><a href="<c:url value="/recruitment/practice/${p}/${a}/${page.number+1}"/>"><i class="icon-forward"></i></a></li>
                    </c:if>
                    <c:if test="${!page.hasNext()}">
                        <li><a href="javascript:void(0)"><i class="icon-forward"></i></a></li>
                    </c:if>
                    <li><a href="<c:url value="/recruitment/practice/${p}/${a}/${page.totalPages-1}"/>"><i class="icon-step-forward"></i></a></li>
                </ul>
            </div>
        </div>
        </div>
