        ...
        <div class="row">
            <div class="col-md-4 ">
                <form  role="form" class="js_base_form" id="materialAddForm">
                    <input type="hidden" name="id" value="${materialResult.id}"/>
                    <div class="form-body">
                        <div class="row">
                            <div class="col-md-12 ">
                    ...
                    <a type="button" class="btn default" href="<c:url value='/admin/workMaterial/index'/>">返回列表</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    $(function () {
        ...
        $('button.js_save_chg').on('click',function () {
            if(validateForm.valid()){
                ...
                if(dataObj.id == ''){
                    var _arr = new Array();
                    $('div.js_specification input').each(function (k, v) {
                    ...
                    if (_arr.length==0){
                        layer.msg('请至少输入一种特征描述');
                        return;
                    }
                    dataObj.specificationArrayStr = JSON.stringify(_arr);
                    url = '<c:url value="/admin/workMaterial/insert"/>';
                }else{
                    url = '<c:url value="/admin/workMaterial/update"/>';
                }
                    ...
                    $.post(url, dataObj, function (result) {
                        ...
                        if (result.success) {
                            layer.msg('操作成功',{time:800},function () {
                                <%--window.location.href='<c:url value="/admin/workMaterial/index"/>';--%>
