...
<div class="portlet light">
    <div class="portlet-title">
        <div class="caption">
            ...
            <span class="caption-subject font-green sbold uppercase">出库明细</span>
        </div>
    ...
    <form class="navbar-form" role="search">
        ...
        <div class="row">
            ...
            <div class="form-group">
                <input type="text" name="outStorageNo" class="form-control" value="" placeholder="出库单号">
            </div>
            <div class="form-group">
                <input type="text" name="itemReceiver" class="form-control" value="" placeholder="领用人">
            </div>
            <div class="form-group">
                <input type="text" name="itemReceiverUnit" class="form-control" value="" placeholder="领用单位">
            </div>
            <div class="form-group">
                <input type="text" name="projectHouseNum" class="form-control" value="" placeholder="栋号">
            </div>
...
$(function () {
    ...
    var treeObj = null;
            ...
            var  e = function(args) {
                ...
                table.init({
                    ...
                    dataTable: {
                        ...
                        pageLength: 10,
                        ajax: {
                            url: "<c:url value="/admin/inventory/outStorage/listDetailData"/>"
                        },
                        order: [[1, "asc"]],
                        columns:[
                            {"data":"id"},
                            ...
                            {"data":"itemReceiver","render" : function(data, type, full, meta ){
                                return data==undefined?'':data;
                            }},
                            {"data":"itemReceiverUnit","render" : function(data, type, full, meta ){
                                return data==undefined?'':data;
                            }},
                            {"data":"projectHouseNum","render" : function(data, type, full, meta ){
                                return data==undefined?'':data;
                            }},
                            {"data":"use","render" : function(data, type, full, meta ){
                                return data==undefined?'':data;
                            }},
                            ...
                            var sumPrice = 0;
                            api.column(13).nodes().each(function(cell, i) {
                            ...
                            api.column(18).nodes().each(function(cell, i) {
                                sumPrice+= parseFloat(cell.innerHTML);
                            });
                            var tr = '<tr><td colspan="13"></td><td name="num">'+num.toFixed(4)+'</td><td colspan="2"></td><td name="inStorageTax">'+tax.toFixed(2)+'</td><td name="inStorageExcTaxSumPrice">'+excTaxSumPrice.toFixed(2)+'</td><td name="sumPrice">'+sumPrice.toFixed(2)+'</td><td></td></tr>';
        ...
        $('button.js_search').click(function () {
            ...
            table.setAjaxParam('itemReceiver',$.trim($('input[name=itemReceiver]').val()));
            table.setAjaxParam('itemReceiverUnit',$.trim($('input[name=itemReceiverUnit]').val()));
        ...
        $('button.js_reset').click(function () {
            ...
            $('input[name=itemReceiverUnit]').val('');
...
//                _form.attr('target', '_blank');
    ...
    $('button.js_download').on('click',function () {
        layer.confirm('确定导出？',function (_index) {
            ...
            $('#downloadForm').remove();
            var _form = $('<form>');
            _form.attr('action','<c:url value="/admin/inventory/outStorage/exportDetailData"/>');
            _form.attr('id','downloadForm');
            _form.attr('method','post');
            ...
            for(var i in _arr){
                var obj = _arr[i];
                if(obj.value != ''){
                    _form.append($('<input>').attr('name',obj.name).val(obj.value));
                }
            }
            $(document).find('body').append(_form);
            $('#downloadForm').submit();
