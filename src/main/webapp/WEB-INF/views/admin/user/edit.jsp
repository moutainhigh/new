                ...
                <form id="user_edit_form" role="form">
                    ...
                    <div class="form-group">
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <c:forEach var="role" items="${roles}">
                                    <label class="control-label ${role.key}"><input type="radio" name="regSource" value="${role.key}" <c:if test="${entity.regSource eq role.key}">checked</c:if>>${role.key}</label>
                                </c:forEach>
                            </c:otherwise>
                        </c:choose>
                        </div>
                    ...
                    <div class="form-actions">
                        ...
                        <a href="<c:url value="/admin/user/list"/>" class="btn btn-default">返回</a>
                    </div>
                </form>
            </div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        form.validate({
            ...
            submitHandler : function(form) {
                ...
                if ($(form).find('input[name=userId]').val()){
                    $.post('<c:url value="/admin/user/userUpdate"/>',$(form).serialize(),function (result) {
                        layer.close(_index);
                        if (result==0){
                            layer.msg("修改失败");
                        }else {
                    ...
                    $.post('<c:url value="/admin/user/userRregistration"/>',$(form).serialize(),function (result) {
                        layer.close(_index);
                        if (result==0){
                            layer.msg("保存失败");
                        }else if(result==2){
        ...
        $('input[name=regSource]').on('click',function () {
            var _reg = $(this).val();
            $('div.js_role label').addClass('hidden');
            $('div.js_role label.'+_reg+'').removeClass('hidden');
        });
