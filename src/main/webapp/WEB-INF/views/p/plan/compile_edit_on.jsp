...
<style>
    ...
    .form_update_save,.form_update_cancel{
        display: none;
    }
    .table .btn{
        margin-top: 5px;
...
<div class="portlet light" id="form_wizard_1">
    ...
    <div class="portlet-title">
        <div class="actions" style="float: left;">
            ...
            <label>版本：</label>
            <input type="text" class="form-control input-inline input-sm" value="${projectArchives.sufVersions}" disabled="disabled" style="width: 30px;"/>
            <c:if test="${projectArchives.isCompile eq 2&&projectArchives.isOn eq 1}">
                <button type="button" class="btn btn-sm green" style="margin-left: 80px;" id="id_on"> 生效 </button>
            </c:if>
        </div>
    </div>
                    ...
                    <c:forEach items="${projectPlanCompileLogList}" var="e" varStatus="status">
                        <tr>
                            <form action="#">
                                ...
                                <td ><input type="text" name="startDate" class="form-control input-inline form_datetime laydate-icon form_update" style="width: 120px;text-align: center;" value="${fn:substring(e.startDate,0,10)}" readonly="readonly" onclick="laydate({istime: true, format: 'YYYY-MM-DD'})" autocomplete="off" data-title="${fn:substring(e.startDate,0,10)}"/></td>
                                <td ><input type="text" name="endDate" class="form-control input-inline form_datetime laydate-icon form_update" style="width: 120px;" value="${fn:substring(e.endDate,0,10)}" readonly="readonly" onclick="laydate({istime: true, format: 'YYYY-MM-DD'})" autocomplete="off" data-title="${fn:substring(e.endDate,0,10)}"/></td>
                                <td ><input type="number" min="1" name="preWarnDay" class="form-control input-inline input-sm form_update" style="width: 50px;" value="${e.preWarnDay}" data-title="${e.preWarnDay}"/></td>
                                <td ><input type="number" min="1" name="sufWarnDay" class="form-control input-inline input-sm form_update" style="width: 50px;" value="${e.sufWarnDay}" data-title="${e.sufWarnDay}"/></td>
                                <td class="hide"><input type="number" min="1" name="weight" class="form-control input-inline input-sm form_update" style="width: 50px;" value="${e.weight}" data-title="${e.weight}"/></td>
                                <td >
                                    <input type="text" class="form-control input-inline input-sm" style="width: 150px;" value="${e.shortDepartment}" readonly="readonly" title="${e.department}" data-title="${e.department}"/>
                                    <input type="hidden" class="form_update" name="department" value="${e.department}" data-title="${e.department}"/>
                                    <input type="hidden" class="form_update" name="departmentId" value="${e.departmentId}" data-title="${e.departmentId}"/>
                                </td>
                                <td >
                                    <input type="text" class="form-control input-inline input-sm search-icon js_choose_emp2 form_update" style="width: 150px;" value="${e.shortWarnName}" readonly="readonly" title="${e.warnName}" data-title="${e.warnName}"/>
                                    <input type="hidden" class="form_update" name="warnName" value="${e.warnName}" data-title="${e.warnName}"/>
                                    <input type="hidden" class="form_update" name="warnUserId" value="${e.warnUserId}" data-title="${e.warnUserId}"/>
                                </td>
                                <td ><input type="text" name="remark" class="form-control input-inline input-sm form_update" style="width: 150px;" value="${e.remark}" placeholder="备注" data-title="${e.remark}"/></td>
                                <td >
                                    <c:if test="${e.isComplete eq 1}">
                                        <button type="button" class="btn btn-xs grey-cascade form_update_cancel" >取消</button>
                                        <button type="button" class="btn btn-xs yellow form_update_save">保存</button>
                                        <button type="button" class="btn btn-xs green-meadow form_update_edit" >编辑</button>
                                    </c:if>
                                </td>
                            </form>
                        </tr>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('.form_update').prop("disabled","disabled");

        $('.form_update_edit').on('click',function () {
            $(this).hide().parent().siblings("td").find('input.form_update').prop("disabled",false);
            $(this).siblings('button').show();
        });

        $('.form_update_cancel').on('click',function () {
            $(this).hide().parent().siblings("td").find('input.form_update').prop("disabled","disabled");
            $(this).parent().siblings("td").find('input.form_update').each(function () {
                $(this).val($(this).data("title")).prop("title",$(this).data("title"))
            });
            $(this).next('button').hide().next('button').show();
        });

        $('.form_update_save').on('click',function () {
            var data = {};
            data.empListStr=encodeEmpData2($(this).parent("td").parent("tr").find("form:eq(0)").serializeArray());
            if (!b){
                layer.confirm("确定保存吗？", {icon: 3, title:'保存提示'}, function(){
                    layer.closeAll();
                    $.ajax({
                        url: '<c:url value="/project/plan/put" />',
                        type: 'POST',
                        data:data,
                        dataType: 'json',
        ...
        $('#id_on').on('click',function () {
            layer.confirm("确定执行生效操作吗？", {icon: 3, title:'生效提示'}, function(){
                layer.closeAll();
                $.ajax({
                    url: '<c:url value="/project/plan/on" />',
                    type: 'POST',
