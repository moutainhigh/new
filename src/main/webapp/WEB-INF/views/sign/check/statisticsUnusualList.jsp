...
<div class="portlet light">
    <div class="portlet-title">
        <div class="caption">
            ...
            <span class="caption-subject font-green sbold ">异常记录报表</span>
        </div>
    </div>
...
<script>
    $(function () {
        ...
        $('button.js_search').on('click',function () {
            ...
            if(_startDate&&_endDate){
                ...
                $.ajax({
                    ...
                    url: '<c:url value="/admin/sign/listUnususlStatisticsData"/>',
        ...
        var entityKey = 'id,pactCompanyName,plateName,companyName,departmentName,groupName,userNum,birthdate,age,' +
            'joinCompDate,inDueFormDate,useStatus,name,sex,position,idCard,technicalLevel,userCategory,workStatus,' +
            'performanceSystem,late,leaveEarly,noWork,punishMoney,remark';
        var  buildTb = function (resultList) {
            var _body = $('#datatable_ajax tbody').empty();
            var _lastId = '',_birthdate='',_age=0;
            $.each(resultList,function (k,v) {
                ...
                $.each(entityKey.split(','),function (i, e) {
                    ...
                    if(e == 'birthdate'){
                        ...
                        if(_birthdate) {
                            _age=jsGetAge(_birthdate);
                            _tr.find('td:eq(8)').text(_age);
                        }
                    } /*else if (e == 'punishMoney') {
                        _tr.find('td:eq(23)').append('<input type="text" name="punishMoney" style="width: 100%"/>');
                    } else if (e == 'remark') {
                        _tr.find('td:eq(24)').append('<input type="text" name="remark"  style="width: 100%"/>');
                    }*/
                });
                _body.append(_tr);
        ...
        $('button.js_export').on('click', function () {
            ...
            if(_startDate&&_endDate){
                $('#printForm').remove();
                var _form = $('<form>');
                _form.attr('action','<c:url value="/admin/sign/exportUnusualSignStatistics"/>');
