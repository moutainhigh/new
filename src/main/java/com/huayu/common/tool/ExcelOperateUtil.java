...
public class ExcelOperateUtil {
    ...
    public static Workbook readExcel2Workbook(String fileName, InputStream inputStream) throws Exception {
        if(fileName.endsWith(".xls")){
             return getHSSFWorkBookByStream(inputStream);
        }else if(fileName.endsWith(".xlsx")){
            //有可能创建失败
            try {
                return getXSSFWorkBookByStream(inputStream);
            } catch (Exception e) {
              return null;
            }
        }else{
    ...
    public static List<String> getRowDataList(Row row, int lastCellNum) {
        ...
        if (row == null) {
            ...
            for (int i = 0; i < lastCellNum; i++) {
                datas.add(getCellData(row.getCell(i)));
            }
        }
        return datas;
    }
    ...
    public static void createCell(Row row, CellStyle cs, int cellIndex, String ... values){
    ...
    public  static void createNullCell(Row row, CellStyle cs, int cellIndex, int length){
        String NULL = null;
        for (int i=0;i<length;i++){
            Cell cell = row.createCell(cellIndex+i);
    ...
    public static void writeFiles(HttpServletResponse response, Workbook workbook, String fileName) throws IOException {
        response.reset();
